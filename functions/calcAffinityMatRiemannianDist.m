function [K, nnData] = calcAffinityMatRiemannianDist(X, configParams)
...
%
% Gal Mishne & Hadas benisty

for k1=1:size(X,3)
    for k2=1:size(X,3)
    Rdist(k1,k2) = RiemannianDist(X(:,:,k1), X(:,:,k2));
    end
end
dParams.kNN       = 4;
dParams.self_tune = 0;
...
[Dis,Inds]=sort(Rdist,'ascend');
nnData.nnDist = Dis(1:configParams.kNN, :);
nnData.nnInds = Inds(1:configParams.kNN, :);
