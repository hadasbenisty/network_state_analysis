% Fig 1 - Behavior
function main_make_behavior_figs

animals={'xw','xx','xz','xt','xu' 'xs'};
stateslabels = {'low_pup_q', 'high_pup_q', 'high_pup_l'};
outputfiggolder = 'X:\Hadas\Meso-imaging\lan\meso_results\figs\behavior';
contrast_levels = [0 2 5 10 20 40 100];
daysmax=30;
plot_time_spent(animals, stateslabels, 'behavior',daysmax)
plot_psych_curve_per_state(animals, stateslabels, contrast_levels, outputfiggolder,daysmax)
...
function plot_psych_curve_per_state(animals, statenames, contrast_levels, outputfiggolder,daysmax)
suc_rate=nan(length(statenames), length(animals));
if exist('X:\Hadas\Meso-imaging\lan\meso_results\ProcessingDirectory\psych_curv_data.mat', 'file')
    load('X:\Hadas\Meso-imaging\lan\meso_results\ProcessingDirectory\psych_curv_data.mat', 'psych_curv', 'suc_rate');
else
    ...
    save('X:\Hadas\Meso-imaging\lan\meso_results\ProcessingDirectory\psych_curv_data.mat', 'psych_curv', 'suc_rate');
...
mysave(gcf, fullfile(outputfiggolder,'c50_per_state'));
makepsychbarplot(rmax,animals,statenamesstr,'R_{max}')
mysave(gcf, fullfile(outputfiggolder,'rmax_per_state'));
makepsychbarplot(baseline,animals,statenamesstr,'baseline')
mysave(gcf, fullfile(outputfiggolder,'baseline_per_state'));
...
mysave(gcf, fullfile(outputfiggolder,'psych_curves_by_state'));
...
mysave(gcf, fullfile(outputfiggolder,'psych_curve_hyperbolicfit_states'));
...
ylabel('% Success Rate');
mysave(gcf, fullfile(outputfiggolder,'success_rate_by_state' ));
end
...
function plot_time_spent(animals, stateslabels, outputfiggolder,daysmax)
mkNewDir(outputfiggolder);
behavelabel = {'correct','incorrect'};
if exist('X:\Hadas\Meso-imaging\lan\meso_results\ProcessingDirectory\time_spent_data.mat', 'file')
    load('X:\Hadas\Meso-imaging\lan\meso_results\ProcessingDirectory\time_spent_data.mat','Nall',...
    ...
    save('X:\Hadas\Meso-imaging\lan\meso_results\ProcessingDirectory\time_spent_data.mat','Nall',...
        'Ncor','Ninc','pupvals','wheelvals');
end
n=length(animals);
CondColors = get_3states_colors;
...
mysave(gcf, fullfile(outputfiggolder, 'trial_wheel_hist_3states'));
...
mysave(gcf, fullfile(outputfiggolder, 'trial_pup_histogram_3states'));
...
mysave(gcf, fullfile(outputfiggolder, 'time_spent_bytrials_absolute_numbers'));
...
mysave(gcf, fullfile(outputfiggolder, 'time_spent_bytrials'));
