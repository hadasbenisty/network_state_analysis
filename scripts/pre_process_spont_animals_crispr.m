...
if isfile(fullfile(procdatapath, animalpath, 'arousal_traces_states.mat'))
    temp=load(fullfile(procdatapath, animalpath, 'arousal_traces_states.mat'));
    if ~isfield(temp.segments_arousals, 'low_face')||~isfield(temp.segments_arousals, 'high_face')
        disp('loaded no facemap fields')
    elseif isempty(temp.segments_arousals.low_face)||isempty(temp.segments_arousals.high_face)
        disp('loaded empty facemap')
    else
        disp('Facemap All good!')
    end
    disp('exist');
    %return; %commented out may 20 temporarily to process facemap
end
...
if  isempty(pupilfile) || length(timing.pupilcamstart) < 1000
    ...
    %added by lav may 27,2021 to standardize with sweyta's normalization
    ...
    MeanF0=mean(F0Vals);
    pupil_Norm=(pupil-MeanF0)./std(F0Vals);
    
    %removed by lav may 27,2021 to standardize with sweyta's normalization
...
if isempty(facemapfile) || ~ispupil
    ...
    %This code adjusts the proc outputs for all sessions, flipping the face whisker PC1 signs based on skewness sign
    ...
    F0Vals = Ms(1:ceil(length(Ms)*0.1)); % lower 10% of the values
    MeanF0=mean(F0Vals);
    face_Norm=(face-MeanF0)./std(F0Vals);
    
    figure;
    subplot(2,1,1); plot(pupil_Norm); title('NormPupil');
    subplot(2,1,2); plot(face_Norm);title('NormFacePC1_Corr');
    mysave(gcf, fullfile(procdatapath, animalpath, 'new normalization'));
    
    
    b1DoPlot=1; blDoPlotDuration=1:700; smoothWin=1;
    ...
    if length(face_Norm)>length(pupil_time)
        face_Norm=face_Norm(1:length(pupil_time));
    end
    ...
    title('facemap high arousal');
    mysave(h3, fullfile(procdatapath, animalpath, 'highface_detection'));
    ...
    title('facemap low arousal');
    mysave(h4, fullfile(procdatapath, animalpath, 'lowface_detection'));
...
if isfacemap
    if size(face,1)<size(pupil_time,1)
        face_time=pupil_time(1:size(face,1));
    elseif size(face,1)>size(pupil_time,1)
        face=face(1:size(pupil_time,1));
        face_time=pupil_time;
    else
        face_time=pupil_time(1:size(face,1));
    end
...
if ~isfield(segments_arousals, 'low_face')||~isfield(segments_arousals, 'high_face')
    disp('END:no facemap fields')
elseif isempty(segments_arousals.low_face)||isempty(segments_arousals.high_face)
    disp('END:empty facemap')
else
    disp('END:facemap good')
end

if ~exist('pupil_Norm','var')
    pupil_Norm=NaN(1,length(wheel_speed));
elseif isempty(pupil_Norm)
    pupil_Norm=NaN(1,length(wheel_speed));
end

if length(pupil_Norm)>length(t_imaging)
    pupil_Norm=pupil_Norm(1:length(t_imaging))
end

figure;
subplot(3,1,1);plot(t_imaging(1:length(pupil_Norm)), wheel_speed(1:length(pupil_Norm)));ylabel('wheel')
...
if ispupil
    subplot(3,1,2);plot(t_imaging(1:length(pupil_Norm)), pupil_Norm(1:length(pupil_Norm)));ylabel('pupil')
...
if isfacemap
    subplot(3,1,3);plot(t_imaging(1:length(pupil_Norm)), face_Norm(1:length(pupil_Norm)));ylabel('face');ylim([min(face) max(face)+200])
...
if isfacemap
    try
        start_t=(Face_HighArousal_On_final_qu(2)-100);end_t=(start_t+700);
        figure;
        subplot(3,1,1);plot(t_imaging(1:length(pupil_Norm)), wheel_speed(1:length(pupil_Norm)));ylabel('wheel');xlim([start_t end_t]);
        for ti=1:size(segments_arousals.loc,1)
            line([1 1]*segments_arousals.loc(ti,1), get(gca,'YLim'),'Color','g');
            line([1 1]*segments_arousals.loc(ti,2), get(gca,'YLim'),'Color','g');
        end
        ...
        xlim([start_t end_t]);
        if ispupil
            subplot(3,1,2);plot(t_imaging(1:length(pupil_Norm)), pupil_Norm(1:length(pupil_Norm)));ylabel('pupil');xlim([start_t end_t]);
            for ti=1:size(segments_arousals.high_pupil,1)
                line([1 1]*segments_arousals.high_pupil(ti,1), get(gca,'YLim'),'Color','g');
                line([1 1]*segments_arousals.high_pupil(ti,2), get(gca,'YLim'),'Color','g');
            end
        ...
        xlim([start_t end_t]);
        subplot(3,1,3);plot(t_imaging(1:length(pupil_Norm)), face_Norm(1:length(pupil_Norm)));ylabel('face');xlim([start_t end_t]);
        for ti=1:size(segments_arousals.high_face,1)
            line([1 1]*segments_arousals.high_face(ti,1), get(gca,'YLim'),'Color','g');
            line([1 1]*segments_arousals.high_face(ti,2), get(gca,'YLim'),'Color','g');
        end
        ...
        xlim([start_t end_t]);
        mysave(gcf, fullfile(procdatapath, animalpath, 'arousal_traces_states_short'));
    catch
    end
end
%
