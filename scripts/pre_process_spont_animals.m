...
for ir=1:length(animals)
    ...
    for dayy=1:length(unique(days_to_process)) %iterate over psychometric days
        ...
        if exist(datafile,'file')
            load(datafile, 'running_time_traces');
            % state 1: low pup + q           
    ...
    save(strcat('X:\Hadas\Meso-imaging\lan\results\ProcessingDirectory\allen_Slope_Amplitude\',animal,'\',animal,'spon_3states'),...
        'low_pup_q','low_pup_q_t','high_pup_q','high_pup_q_t','high_pup_l','high_pup_l_t','days_to_process');
end
...
for day_i=1:length(days2process)
    ...
    close all;
    save(fullfile('X:\Hadas\Meso-imaging\lan\', [animalName 'psych'], 'spike2Features', [animalName num2str(days2process(day_i)) 'arousal_state_ITI.mat']),  'running_time_traces');
end
