function [outputMat_grabs_regular, outputMat_grabs_ridge] = ...
    returnCVRSquared(dataFolderPath,imagingdatapath,spike2path,airpuff_trial,howManyFacePCs)
tic
%**************************************************************************
...
smrx_sigs = load('smrx_signals_v2.mat');
...
if airpuff_trial
    ...
    time_to_remove_before = 10*10; %s * 10 Hz
    time_to_remove_after = 60*10; %s * 10 Hz
...
toremove=find(all(~isnan(blue_parcels))==0); % find indices where there are missing NaNs for all parcels
