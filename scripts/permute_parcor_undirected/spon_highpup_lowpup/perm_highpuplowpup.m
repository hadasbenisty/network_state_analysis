function perm_highpuplowpup(animal)
rehash
rehash path
...
for kr=1:100
    % load the time traces file (notice the animal and day)
    pupildata=load(strcat(inputfolder, strcat(num2str(kr),animal,'perm_pupilhigh_pupillow.mat')));
    for j=1:2
        if j==1
            data = pupildata.pupilhighalldata(finalindex,all(~isnan(pupildata.pupilhighalldata)));condition='highpup';   % for nan - columns (so all parcels are aligned).
        elseif j==2
            data = pupildata.pupillowalldata(finalindex,all(~isnan(pupildata.pupillowalldata)));condition='lowpup';      % for nan - columns (so all parcels are aligned).
        end
