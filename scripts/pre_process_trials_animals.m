...
isfirst = true;
outputfile_Allen = fullfile(datapath,[animalName  '_trial_imaging_time_traces_global_Allen_dfff.mat']);
outputfile_LSSC = fullfile(datapath,[animalName  '_trial_imaging_time_traces_global_LSSC_dfff.mat']);
outputfile_grid4 = fullfile(datapath,[animalName  '_trial_imaging_time_traces_global_Grid4_dfff.mat']);
outputfile_meta = fullfile(datapath,[animalName '_trial_meta_data.mat']);

if exist(outputfile_Allen, 'file') && exist(outputfile_LSSC, 'file') ...
        &&exist(outputfile_grid4, 'file') &&exist(outputfile_meta, 'file') &&~do_over
    return;
end
...
for day_i=1:length(days2process)
    ...
    load(fullfile(spike2pth, ['spike2data',animalName num2str(days2process(day_i)) '.mat']),...
    ...
    if ~exist(fullfile(datapath,[animalName num2str(days2process(day_i)) 'imaging_time_traces_global_dfff.mat']),'file')
        disp([animalName num2str(days2process(day_i)) ' does not exist']);
        continue;
    end
    load(fullfile(datapath,[animalName num2str(days2process(day_i)) 'imaging_time_traces_global_dfff.mat']),...
        'trialslabels','roiLabelsbyAllen','maskByAllen','regionLabel','imaging_time_traces');
    ...
    if isfirst
        imaging_time_Allen = imaging_time_traces.Allen;
        imaging_time_LSSC = imaging_time_traces.Gal;
        imaging_time_Grid4 = imaging_time_traces.grid4;
        if animalName == 'xs'
           pupil_trace = squeeze(imaging_time_traces.pupil_trace(:,1:236,:));
        wheel_trace = squeeze(imaging_time_traces.wheel_trace(:,1:236,:));
        else 
        pupil_trace = squeeze(imaging_time_traces.pupil_trace);
        wheel_trace = squeeze(imaging_time_traces.wheel_trace);
        end
        behavior_labels =  trialslabels.blinksummary;
        inject_labels = trialslabels.injectioncond;
        contrast_labels =  trialslabels.contrastLabels;
        arousal_labels = trialslabels.trials_labels_arousal_pup_loose;
        days_labels =  ones(length(trialslabels.blinksummary),1)*days2process(day_i);
        isfirst = false;
    else
        imaging_time_Allen = cat(3, imaging_time_Allen, imaging_time_traces.Allen);
        imaging_time_LSSC = cat(3, imaging_time_LSSC, imaging_time_traces.Gal);
        imaging_time_Grid4 = cat(3, imaging_time_Grid4, imaging_time_traces.grid4);
        ...
        behavior_labels = cat(1, behavior_labels, trialslabels.blinksummary);
        inject_labels = cat(1, inject_labels, trialslabels.injectioncond);
        contrast_labels = cat(1, contrast_labels, trialslabels.contrastLabels);
        arousal_labels = cat(1, arousal_labels, trialslabels.trials_labels_arousal_pup_loose);
        days_labels =  cat(1, days_labels, ones(length(trialslabels.blinksummary),1)*days2process(day_i));
    end
end
...
roi_labels = roiLabelsbyAllen.Allen(finalindex);  
roi_location = maskByAllen.Allen;
region_labels = regionLabel.Allen;
roi_names = parcels_names;
imaging_time_traces = imaging_time_Allen;
save(outputfile_Allen, 't', 'imaging_time_traces', 'roi_labels', 'roi_location', 'region_labels', 'roi_names');
roi_labels = roiLabelsbyAllen.Gal;  
roi_location = maskByAllen.Gal;
region_labels = regionLabel.Gal;
roi_names = parcels_names(roiLabelsbyAllen.Gal);
imaging_time_traces = imaging_time_LSSC;
save(outputfile_LSSC, 't', 'imaging_time_traces', 'roi_labels', 'roi_location', 'region_labels', 'roi_names');
roi_labels = roiLabelsbyAllen.grid4;  
roi_location = maskByAllen.grid4;
region_labels = regionLabel.grid4;
roi_names = parcels_names(roiLabelsbyAllen.grid4);
imaging_time_traces = imaging_time_Grid4;
save(outputfile_grid4,'t', 'imaging_time_traces', 'roi_labels', 'roi_location', 'region_labels');
save(outputfile_meta, 'pupil_trace', 'wheel_trace', 'pup_wheel_t', 'behavior_labels', 'inject_labels', ...
