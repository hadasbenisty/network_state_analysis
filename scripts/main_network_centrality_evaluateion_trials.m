...
function eval_weights_and_cent(isloose, animal, statenames, pre_trial_time_start, pre_trial_time_end, topermind) %#ok<DEFNU>
if isloose
    loosestr = 'loose';
else
    loosestr = '';
...
for state_i=1:length(statenames)
    ...
    %semilogx(0:0.01:100,fittedpredictedcurveF,'color',CondColors(state_i,:));
...
save(strcat('X:\Lav\ProcessingDirectory\parcor_undirected\','data4mike'),'CRFinfo');
