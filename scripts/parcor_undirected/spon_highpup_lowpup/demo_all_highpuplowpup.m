function demo_all_highpuplowpup(animal, outputpath, isw, saveplots)
if nargin == 3
    saveplots = false;
end
rehash
...
outputfolder=fullfile(outputpath, animal,'\');
mkNewDir(animal);
...
for j=1:2
    if j==1
        data = pupildata.pupilhighalldata(finalindex,all(~isnan(pupildata.pupilhighalldata)));condition='pupilhigh';   % for nan - columns (so all parcels are aligned).
    elseif j==2
        data = pupildata.pupillowalldata(finalindex,all(~isnan(pupildata.pupillowalldata)));condition='pupillow';      % for nan - columns (so all parcels are aligned).
    end
