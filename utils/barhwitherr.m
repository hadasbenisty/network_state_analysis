...
function varargout = barhwitherr(errors,varargin)
...
handles.bar = barh(varargin{:}); % standard implementation of bar fn
hold on
hBar = handles.bar;
if length(hBar) == 1
hBar.FaceColor = 'w';
   ...
   hBar(1).FaceColor = 'w'; 
   hBar(2).FaceColor = 'k';
   %hBar(2).FaceColor = rgbcolor('gray');
%    hBar(3).FaceColor = 'k';
end
if nRows > 1
    ...
    set(hErrorbar, 'marker', 'none', 'Color','r');
end
