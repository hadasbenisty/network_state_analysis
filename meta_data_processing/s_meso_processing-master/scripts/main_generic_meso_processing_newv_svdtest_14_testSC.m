...
function [dFoF,dFoF_parcells,R,C]= main_generic_meso_processing_newv_svdtest_14_testSC(tiffsPath, dataSmrxFile,datsStimFile, outputPath, TmpFile,channels,params)
...
if exist(fullfile(outputPath, 'final_dFoF.mat'), 'file')
    ...
    %run spatial svd with a patch size of 18 
    for i = 1:length(names)
        if  ~strcmp(names{i},'uv')
           if exist(fullfile(outputPath, strcat('SVDOut',names{i},'.mat')), 'file') % if the output already exists load that % if the output already exists load that 
            ...
            [output_sig, sig_blue, sig_uv] = spatial_regression_fast_testSC(dFoF.(names{i}), dFoF.uv, mask, 14, 0);
