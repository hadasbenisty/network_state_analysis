...
switch paramsSignalsExtraction.sigs
    case 'blueuv'
        ...
        firstCaFrame = paramsSignalsExtraction.firstCaFrame;
        ...
        secondframe=mov(:,:,2);
        lastframe=mov(:,:,end);
        secondlastframe=mov(:,:,end-1);
        
        firstframe_linear=double(reshape(firstframe(68:188,68:188),[],1));
        secondframe_linear=double(reshape(secondframe(68:188,68:188),[],1));
        lastframecorr=corr(double(reshape(lastframe(68:188,68:188),[],1)),secondframe_linear);
        secondlastframecorr=corr(double(reshape(secondlastframe(68:188,68:188),[],1)),secondframe_linear);
        % larger is the uv channel
        if lastframecorr>secondlastframecorr
            lastframe=secondlastframe;
        end
...
%         sigsMov.blue =  mov(:, :, firstCaFrame:hop:end);
%         sigsMov.uv =  mov(:, :, ratio+firstCaFrame:hop:end);
    case 'RCaMP_AC'
