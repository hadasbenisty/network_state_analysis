function [output_sig, sig_blue, sig_uv] = spatial_regression_fast(blue_sig, uvsig, brain_mask, patch_size, delay_length) 
...
[sig_blue, sig_uv, output_sig]  = regress_filter_fast(rmmean_blue(:, delay_length+1:end), rmmean_uv(:, delay_length+1:end), brain_mask, patch_size);
output_sig = cat(2, nan(size(output_sig, 1), delay_length), output_sig); 
output_sig=output_sig+mean_blue; %adding the mean singal back 